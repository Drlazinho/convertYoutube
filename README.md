...
- **Busca Integrada Ultrarrápida:** Pesquise músicas e artistas diretamente na barra de pesquisa. A busca utiliza leitura plana (flat-playlist) para retornar dezenas de resultados em segundos, com sistema inteligente de paginação local para evitar latência.
- **Gerenciador de Fila Simultâneo:** Adicione múltiplos downloads ao mesmo tempo sem travar a interface. Uma gaveta flutuante monitora o progresso (velocidade e ETA) de cada arquivo isoladamente, com proteção contra fechamento acidental da janela.
- **Metadados e Capas Nativas (ID3 Tags):** O aplicativo embute automaticamente informações do YouTube e converte a thumbnail original em capa de álbum (JPG) nativamente no arquivo, exibindo-a em qualquer media player e dispositivo automotivo.
- **Download de Vídeos H.264 Universal:** Suporte para baixar vídeos do YouTube em 1080p (Full HD), 720p, 480p e 360p com mesclagem de áudio e vídeo usando FFmpeg. Arquivos finais são estritamente forçados ao codec H.264 (avc1) em MP4, garantindo reprodução impecável e sem telas pretas nas TVs, WhatsApp e media players antigos.
...
- **Autenticação Nativa Profissional (Deep Linking):** Sistema de login robusto via Supabase que impede acessos não autorizados. Os usuários podem autenticar via E-mail/Senha ou provedores OAuth (Google, GitHub) através do navegador padrão do sistema operacional, com redirecionamento contínuo de volta para a aplicação desktop (`converttube://`), garantindo segurança máxima e prevenindo sequestro de sessão em computadores compartilhados.
